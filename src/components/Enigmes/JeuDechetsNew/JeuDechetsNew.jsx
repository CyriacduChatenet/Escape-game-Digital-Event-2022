...
export const JeuDechetsNew = () => {
  ...
  const [dechetsList, setDechetsList] = useState([
    {name: "clope", trouve:false, img: "/assets/images/jeu-dechets/megot-cigarette.png", className: "dechet2", collect: false},
    {name: "canette", trouve:false, img: "/assets/images/jeu-dechets/canette.png", className: "dechet4", collect: false},
  ])
      ...
      <div className="inventaire">
        ...
        <div className="section-dechet-inventaire">
          {dechetsList.map(dechet => (
                <p className="dechet-inventaire" style={{color: dechet.collect ? 'green' : 'red'}}>{dechet.trouve ? dechet.name : "?"}</p>
            ))}
        </div>
      </div>
          ...
          {dechetsList.map((dechet, index) => (
            <>
              {dechet.trouve ? (
                ...
                <img src="/assets/images/jeu-dechets/dechet-a-decouvrir.png" className={"dechet " + dechet.className} />
              )}
            </>
          ))}
